...
var HMAC = require('../src/plugins/hmac.js');
...
describe('HMAC', function() {
  describe('KDF Spec', function() {
    ...
    it('should expose the default spec', function() {
      expect(HMAC.defaultSpec).to.eql({
        algorithm: 'HMAC',
        hash: "SHA256",
    ...
    it('should fail with a wrong algorithm', function() {
      expect(function() {
        new HMAC({ algorithm: 'HMAK' });
      }).to.throw("Algorithm must be HMAC");
    });

    it('should fail with a wrong hash', function() {
      expect(function() {
        new HMAC({ hash: 'sillyhash' });
  ...
  describe('simple key derivation', function() {
    ...
    it('should hash and verify a password', function(done) {
      var spec = { algorithm: 'HMAC', hash: 'SHA512', derived_key_length: 32 };

      new HMAC(spec).deriveKey('password', function(err, key1) {
        if (err) return done(err);
        try {
          expect(key1.derived_key.length).to.equal(32);
          expect(key1.kdf_spec).to.eql(spec);
          new HMAC(key1.kdf_spec).deriveKey('password', key1.salt, function(err, key2) {
            if (err) done(err);
            try {
  ...
  describe('RFC-4231 test vectors', function() {
    for (var i = 0; i < vectors.length; i++) (function(test, vector) {
      var hashes = [ 'sha224', 'sha256', 'sha384', 'sha512' ];
      for (var j in hashes) (function(hash) {

        it('should validate test case ' + test + ' for ' + hash, function(done) {

          new HMAC({ hash: hash, derived_key_length: (vector[hash].length / 2) })
            .deriveKey(new Buffer(vector.data, 'hex'), new Buffer(vector.key, 'hex'), function(err, key) {
              try {
                expect(key.derived_key.toString('hex')).to.equal(vector[hash]);
                done(err);
              } catch (error) {
                done(error);
              }
